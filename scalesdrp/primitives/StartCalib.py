...
########################## reference pixel and 1/f correction ######################    
    def reffix_hxrg(self,cube, nchans=4, in_place=False, fixcol=True, **kwargs):
        ...
        cube = self.reffix_amps(cube, nchans=nchans, in_place=True, **kwargs)
        # Fix 1/f noise using vertical reference pixels
        if fixcol:
            cube = self.ref_filter(cube, nchans=nchans, in_place=True, **kwargs)
        return cube

    def reffix_amps(self,cube, nchans=4, in_place=True, altcol=True, supermean=False,
        ...
        refs_amps_avg = self.calc_avg_amps(refs_all, cube.shape, nchans=nchans, altcol=altcol, **kwargs)
    ...
    def ref_filter(self,cube, nchans=4, in_place=True, avg_type='frame', perint=False, 
        ...
        refvals = self.calc_avg_cols(refs_left, refs_right, avg_type, **kwargs)
        # The delta time doesn't seem to make any difference in the final data product
        delt = 10E-6 * (nx/nchans + 12.)
        refvals_smoothed = self.calc_col_smooth(refvals, cube.shape, perint=perint, 
    ...
    def calc_avg_amps(self,refs_all, data_shape, nchans=4, altcol=True, mean_func=robust.mean, **kwargs):
    ...
    def calc_avg_cols(self,refs_left=None, refs_right=None, avg_type='frame',
    ...
    def calc_col_smooth(self,refvals, data_shape, perint=False, edge_wrap=False, 
        ...
        if perint: # per integration
            if edge_wrap: # Wrap around to avoid edge effects
                ...
                if savgol: # SavGol filter
                    ...
                    refvals_smoothed2 = self.smooth_fft(refvals2, delt)
                ...
                if savgol:
                    ...
                    refvals_smoothed = self.smooth_fft(refvals, delt)
            ...
            if edge_wrap: # Wrap around to avoid edge effects
                for ref in refvals:
                    ...
                    if savgol:
                        ...
                        ref_smth = self.smooth_fft(ref2, delt)
                ...
                for ref in refvals:
                    if savgol:
                        ...
                        ref_smth = self.smooth_fft(ref, delt)
                    refvals_smoothed.append(ref_smth)
                refvals_smoothed = np.array(refvals_smoothed)
        return refvals_smoothed

    def smooth_fft(self,data, delt, first_deriv=False, second_deriv=False):
            ...
            if imtype in organized_groups:
                ...
                for group in groups_for_this_type:
                    ...
                    for filename in filenames:
                        ...
                        sci_im_full_original = self.reffix_hxrg(sci_im_full_original1, nchans=4, fixcol=True)
                        self.logger.info("+++++++++++ refpix & 1/f correction completed +++++++++++")
